...
namespace CinemaSeatingSimulation
{
    public partial class FormSimulationScreen : Form
    {
        ...
        private void FormLayoutTry1_Load(object sender, EventArgs e)
        {
            Hall hall = new Hall();
            hall.ConfigHall();
            Door[] doorResult = hall.door();
            Panel pDoors = new Panel();
            for (int i = 0; i < doorResult.Count(); i++) {
                pDoors.Location = new Point(doorResult[i].PosX, doorResult[i].PosY);
                pDoors.Size = new Size(doorResult[i].DWidth, doorResult[i].DHeight);
                pDoors.BackColor = Color.Blue;
                this.Controls.Add(pDoors);
            }
...
>>>>>>> Greg:CinemaSeatingSimulation/FormLayoutTry1.cs
        }
